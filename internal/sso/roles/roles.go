...
// ProfileSettings provides the profile-related configuration that roles need.
// It is implemented by *sso.Settings.
...
func (r *Roles) GetRole(accountId int64, roleName string) (*AWSRoleFlat, error) {
	...
	role, ok := account.Roles[roleName]
	if !ok {
...
func (r *Roles) GetRoleByProfile(profileName string, s ProfileSettings) (*AWSRoleFlat, error) {
...
// CheckProfiles verifies that all the Profile names are unique for all the defined roles.
func (r *Roles) CheckProfiles(s ProfileSettings) error {
	...
	for accountId, account := range r.Accounts {
		for roleName, role := range account.Roles {
			...
			if arn, duplicate := profileUniqueCheck[pname]; duplicate {
				...
				profileUniqueCheck[pname] = role.Arn
			}
		}
	}
	return nil
...
// ProfileName returns either the user-defined Profile value for the role from
// the config.yaml or the generated Profile using the ProfileFormat template
func (r *AWSRoleFlat) ProfileName(s ProfileSettings) (string, error) {
	if len(r.Profile) > 0 {
		return r.Profile, nil
	}

	format := s.GetProfileFormat()
	if len(format) == 0 {
...
// GetEnvVarTags returns a map containing a set of keys representing the
// environment variable names and their values
func (r *AWSRoleFlat) GetEnvVarTags(s ProfileSettings) map[string]string {
