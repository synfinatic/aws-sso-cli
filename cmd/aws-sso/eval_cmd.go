...
type EvalCmd struct {
	...
	OverwriteEnv bool   `kong:"short='O',help='Force overwriting existing AWS_DEFAULT_REGION/AWS_REGION environment variables'"`
...
func (cc *EvalCmd) Run(ctx *RunContext) error {
	...
	region := ctx.Settings.GetDefaultRegion(accountid, role, ctx.Cli.Eval.NoRegion, ctx.Cli.Eval.OverwriteEnv)
